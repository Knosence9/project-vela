...
Select at most 32 fully validated records in deterministic relative-path order, optionally restricted by schema-v1 metadata and exact authored evidence, with:
...
nix develop --command cargo run --locked -p vela-dev -- corpus sample corpus/development 32 --outcome-summary "Validation works"
nix develop --command cargo run --locked -p vela-dev -- corpus sample corpus/development 32 --verification-command "just verify"
nix develop --command cargo run --locked -p vela-dev -- corpus sample corpus/development 32 --attempt-diagnostic "failed check"
nix develop --command cargo run --locked -p vela-dev -- corpus sample corpus/development 32 --attempt-patch "crates/vela-dev/src/lib.rs"
nix develop --command cargo run --locked -p vela-dev -- corpus sample corpus/development 32 --lesson "Keep validation deterministic"
#+end_src

Sampling validates the entire discovered corpus before emitting one compact JSON array of relative paths and complete records. Limits must be from 1 through 1024. The =--example= filter accepts exactly =positive= or =negative=; =--attempt-outcome= accepts exactly =success=, =failure=, or =blocked= and matches a record when any attempt has that outcome; =--verification-status= accepts exactly =passed=, =failed=, or =not-run= and matches when any =outcome.verification[]= check has that status; and =--verified=, =--sanitation-passed=, =--has-sanitation-blockers=, =--has-lessons=, =--has-attempt-diagnostic=, =--has-rejection-rationale=, =--has-attempts=, and =--has-verification= each accept exactly =true= or =false=. The first two boolean filters match the aggregate =outcome.verified= and =sanitation.passed= decisions; the blocker, lesson, attempt-presence, and verification-presence filters match whether =sanitation.blockers=, =lessons=, =attempts=, and =outcome.verification= respectively are non-empty; the diagnostic filter matches whether any =attempts[].diagnostic= field is non-null; and the rationale filter matches whether =example.rejection_rationale= is non-null. These predicates are independent: aggregate decisions are not inferred from individual checks or blocker presence, blocker presence is not inferred from =sanitation.passed=, lesson, attempt, and verification presence do not inspect or infer entry contents, diagnostic presence does not inspect, trim, normalize, or infer diagnostic text or attempt outcomes, and rationale presence does not inspect, trim, normalize, or infer rationale text or example type. Records without the corresponding entries do not match existential attempt or verification filters; a record without attempts has neither attempt presence nor an attempt diagnostic. The optional =--repository-path= filter matches =provenance.repository_path= by exact, case-sensitive UTF-8 equality; it does not trim, normalize separators, resolve paths, inspect the filesystem, or infer related files. The optional =--provenance-url= filter likewise matches =provenance.url= by exact, case-sensitive UTF-8 equality; it does not trim, normalize, parse, fetch, or otherwise dereference the URL. The optional =--task-title= filter matches =task.title= by exact, case-sensitive UTF-8 equality; it does not trim, normalize, tokenize, score, or interpret title text. The optional =--task-objective= filter likewise matches =task.objective= exactly and does not trim, normalize, tokenize, score, or interpret objective text. The optional =--acceptance-criterion= filter matches when any =task.acceptance_criteria[]= entry equals the caller's UTF-8 value exactly and case-sensitively; it does not trim, normalize, perform fuzzy matching, or infer related criteria. All filters compose conjunctively before the limit without changing path order. Empty matches succeed with =[]=; traversal, record, JSON, and semantic failures produce no partial stdout, and record diagnostics use quoted, debug-escaped relative paths. Sampling is read-only and uses no randomness, ambient state, network access, scoring, indexing, or filesystem authority beyond reading discovered records.

The optional =--attempt-summary= filter matches when any =attempts[].summary=
equals the caller's UTF-8 value exactly and case-sensitively. It does not trim,
normalize, perform fuzzy matching, or infer from diagnostics, patches, outcomes,
or other fields. Records without attempts do not match. The predicate composes
conjunctively before the limit and grants no additional filesystem or network
access.

The optional =--outcome-summary= filter matches =outcome.summary= against the
caller's UTF-8 value exactly and case-sensitively. It does not trim, normalize,
perform fuzzy matching, or infer from attempt summaries, diagnostics,
verification commands, verified state, or other fields. The predicate composes
conjunctively before the limit and grants no additional filesystem or network
access.

The optional =--verification-command= filter matches when any
=outcome.verification[].command= equals the caller's UTF-8 value exactly and
case-sensitively. It does not trim, normalize, tokenize, execute, or infer
related commands. Records without verification entries do not match. The
predicate composes conjunctively before the limit and grants no process,
filesystem, or network access.

The optional =--attempt-diagnostic= filter matches when any non-null
=attempts[].diagnostic= equals the caller's UTF-8 value exactly and
case-sensitively. It does not trim, normalize, perform fuzzy matching, or infer
from attempt summaries, patches, outcomes, or other fields. Records without
attempts, and attempts whose diagnostics are all null, do not match. The
predicate composes conjunctively before the limit and grants no process,
filesystem, or network access.

The optional =--attempt-patch= filter matches when any =attempts[].patch=
equals the caller's UTF-8 value exactly and case-sensitively. It does not trim,
normalize, resolve, inspect, or infer paths or patch evidence. Records without
attempts do not match. The predicate composes conjunctively before the limit
and grants no additional process, filesystem, or network access.

The optional =--lesson= filter matches when any =lessons[]= entry equals the
caller's UTF-8 value exactly and case-sensitively. It does not trim, normalize,
perform fuzzy matching, or infer lessons from other evidence. Records without
lessons do not match. The predicate composes conjunctively before the limit and
grants no additional process, filesystem, or network access.
