...
Select at most 32 fully validated records in deterministic relative-path order, optionally restricted to one schema-v1 trust level, example type, and attempt outcome, with:
...
Sampling validates the entire discovered corpus before emitting one compact JSON array of relative paths and complete records. Limits must be from 1 through 1024. The =--example= filter accepts exactly =positive= or =negative=; =--attempt-outcome= accepts exactly =success=, =failure=, or =blocked= and matches a record when any attempt has that outcome. Records without attempts do not match an attempt-outcome filter. All filters compose conjunctively without changing path order or limit behavior. Empty matches succeed with =[]=; traversal, record, JSON, and semantic failures produce no partial stdout, and record diagnostics use quoted, debug-escaped relative paths. Sampling is read-only and uses no randomness, ambient state, network access, scoring, or indexing.
