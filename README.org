...
Inspection prints each valid record with a quoted, debug-escaped relative path and an aggregate summary. It continues past malformed, unreadable, or semantically invalid records, emits diagnostics prefixed by the same control-safe path field, and exits non-zero when any record is invalid.
