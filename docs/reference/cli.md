...
- `vela cron --report` summarizes durable scheduler state across jobs, running/overdue/lease-expired health signals, outcomes, recoveries, and delivery status, including delivered-delivery counts plus bounded per-job schedule/source, lease/session ownership context (`updated_at`, `last_recovered_at`, `last_session_id`, `execution_token`, `lease_expires_at`), due state / health lag seconds, delivery timestamps, delivery event types, and delivery error excerpts without inspecting raw JSON; `vela cron --show <id>` now surfaces the same ownership fields for one job
