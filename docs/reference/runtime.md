...
- `vela chat --query ...` creates a single-turn session and can call a configured runtime provider for text turns, with Ollama, a deterministic mock backend, a bounded llama.cpp backend, and an embedded in-process contract now serving as explicit provider implementations or targets behind that boundary
