...
- scheduled jobs now surface explicit progression states in CLI status (`registered`, `started-attempt`, `recovered-for-retry`, `completed-rescheduled`, `failed-rescheduled`) plus missed-run accounting (`missed_run_count`, `last_missed_run_count`), explicit delivery progression semantics (`delivery-configured`, `delivery-unconfigured`, `delivery-pending`, `delivery-delivered`, `delivery-failed`, `delivery-skipped`), and bounded delivery evidence (`delivery_webhook_url`, `delivery_event_type`, `last_delivery_outcome`, `last_delivery_status_code`, `delivery_attempt_count`, `last_delivery_error`) so recurring behavior is visible without reading raw timestamps alone
- `vela cron --report` now aggregates durable scheduler visibility into one bounded summary (job counts, running/overdue/lease-expired health counts, outcomes, delivery failures, delivered deliveries, skipped deliveries, recoveries, total missed recurring intervals, total delivery attempts, and next due job) and prints per-job schedule/source plus lease/session ownership context (`updated_at`, `last_recovered_at`, `last_session_id`, `execution_token`, `lease_expires_at`), due state / health lag seconds, missed-run totals, last-run / last-failure details, delivery timestamps, delivery event types, delivery progression, delivery attempt counts, delivery status codes, and bounded delivery/error excerpts so operators can inspect recurring health without scanning every job row or raw JSON
