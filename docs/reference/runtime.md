...
- `vela chat --query ...` creates a single-turn session titled from the query text (`chat: <trimmed query preview>`) and can call a configured runtime provider for text turns, with Ollama, a deterministic mock backend, a bounded llama.cpp backend, and the embedded in-process backend now serving as explicit provider implementations or targets behind that boundary
