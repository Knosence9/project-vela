...
- `vela eval --run-slot ternary-preview ...` now drives the first bounded architecture experiment slot from `~/.vela/evals/slots.json`, recording which slot was used so future routing experiments stay inspectable and reversible; slot inspection surfaces now also show the latest durable eval id/time/result-count/parity summary plus the latest passed backends, failed backends, and capability-group evidence for each slot, and the current published slots carry `embedded` alongside the existing bounded providers so local-first evidence remains visible without mutating the live route
