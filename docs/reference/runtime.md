...
- `vela cron --report` now aggregates durable scheduler visibility into one bounded summary (job counts, outcomes, delivery failures, recoveries, and next due job) so operators can inspect recurring health without scanning every job row
