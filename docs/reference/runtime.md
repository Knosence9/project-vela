...
- `vela extensions --reload` now re-reads extension config + manifest files, recomputes lifecycle transitions, refreshes extension state without resetting durable session state, and fails explicitly when kernel-owned runtime drift is detected, surfacing owned config boundaries such as `runtime.provider@kernel-runtime` plus bounded previous/reloaded value diffs for each blocked setting
- extension reload ownership checks now compare against a durable last-applied kernel-runtime baseline (`~/.vela/runtime/reload-ownership-baseline.json`) so restart-only drift remains enforceable across fresh CLI processes instead of only within one in-memory bootstrap
...
- `vela eval --run-slot ternary-preview ...` now drives the first bounded architecture experiment slot from `~/.vela/evals/slots.json`, recording which slot was used so future routing experiments stay inspectable and reversible, and `capability-parity-scan` now surfaces durable parity summaries that make backend capability differences operator-visible
