...
The additive composed counterparts accept explicit caller policy and exact skill IDs without changing this permission protocol. `execute_composed_provider_tool_step` validates skill selection first and sends system policy, developer policy, selected skills, transcript, and tool metadata as distinct fields. On a completed tool request, the returned continuation retains that exact instruction composition and in-memory result; `continue_composed_provider_tool_step` may refresh descriptive registry metadata but cannot replace the policies, skills, or transcript. Skills, tool metadata, inputs, and outputs never authorize an invocation. Every composed provider-tool-step operation still requires a fresh caller-owned invocation identity and authorizer, invokes at most one tool, and advances only when the caller explicitly continues.
...
`ToolRegistry::unregister_all` atomically removes one caller-owned exact-ID batch. It preflights the entire request before mutation: the lexicographically first duplicate ID takes precedence over lookup, then the lexicographically first absent ID returns `ToolRegistryRemovalError::NotFound`. Either failure is source-free and leaves all adapters unchanged. Empty removal succeeds, and successful removal preserves every unrelated adapter. Mutable registry ownership prevents this synchronous operation from racing an invocation through the same registry; it does not cancel work already executing elsewhere.

`ToolRegistry::replace_all` atomically swaps one homogeneous adapter batch by exact `ToolId`. It applies the same deterministic duplicate-before-missing preflight through the source-free `ToolRegistryReplacementError`; any failure preserves every original adapter. Empty replacement succeeds. Successful replacement changes exactly the supplied IDs and leaves unrelated adapters unchanged in deterministic metadata order. Replacement is explicit and caller-driven: the registry does not discover files, construct adapters, retain rollback history, watch for changes, reload automatically, or persist lifecycle state.

`ToolRegistry::reconcile_all` atomically transitions one caller-owned previous exact-ID set to one homogeneous current adapter batch. It removes previous-only IDs, replaces shared IDs, adds current-only IDs, and preserves unrelated adapters. Preflight rejects duplicate previous IDs, duplicate current IDs, missing previous adapters, then collisions between current-only and unrelated active adapters; each class reports its lexicographically first ID through the source-free `ToolRegistryReconciliationError`. Empty reconciliation succeeds. The registry does not infer ownership: the extension lifecycle layer must supply both explicit selections and construct every current adapter before this commit.
