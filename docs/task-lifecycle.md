...
The `vela-kernel` crate starts Vela's task/session boundary with a deliberately small lifecycle: starting, completing with an output, cancelling, failing with a diagnostic, and loading a task over the typed SQLite event log.
...
- Unknown event discriminators or payload versions and malformed payloads, including an empty persisted completion output or failure diagnostic, remain explicit `ReplayError` values wrapped by `TaskStoreError::Replay`; persisted data is never skipped.
