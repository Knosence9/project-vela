...
Magit context is JSON null unless =magit-status= is already callable and the
current buffer derives from =magit-mode=. Classification traverses at most 32
unique native mode-parent symbols, terminates cycles, and fails closed on
malformed or over-limit ancestry. A recognized buffer reports only the bounded
exact =major_mode= symbol name. This classifies already-loaded current Magit
buffers without loading Magit, calling Git, resolving or selecting a
repository or another buffer, reading section values, exposing paths, refs,
revisions, commands, process state, or buffer text, refreshing status,
navigating, staging, committing, pushing, or granting mutation authority.
...
six-section shape with 128 diagnostics, and cyclic response values fail closed.
The =include= vector accepts each of the six supported sections at most once,
