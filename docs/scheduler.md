...
- `ScheduleStore::list` discovers every schedule from authoritative creation events in one SQLite read snapshot. It returns pending, cancelled, and claimed intents ordered by exact schedule ID, including exact cancellation evidence, while excluding unrelated streams; an empty store returns an empty list.
...
The caller owns every cutoff supplied to `list_due` or `claim`, every cancellation and claim decision, and every action taken from a listed, due, or claimed result. Full discovery is read-only and grants no authority. Cancellation prevents future due selection but does not interrupt work already selected elsewhere. A claim is only a durable reservation: it does not create a task, start or advance a workflow, call a provider, invoke a tool, grant or revoke permission, sleep, retry, or persist execution results.
