...
- `FixedIntervalRecurrence` preserves its exact ID, validated `TaskGoal`, anchor, interval, occurrence count, final occurrence, immutable definition revision, mutable aggregate revision, and optional cancellation. `RecurrenceStore::create` validates that final occurrence before appending one version-1 `recurrence.fixed_interval_created` event with `ExpectedVersion::NoStream`. Exact ranges ending at `u64::MAX` succeed; overflow fails with typed operand evidence before persistence. Duplicate creation preserves the original definition. `load` returns no definition for a missing stream and otherwise requires one strictly decoded, fully representable creation event followed by at most one valid cancellation event. See [ADR-0037](adr/0037-durable-finite-fixed-interval-recurrence-definitions.md) and [ADR-0082](adr/0082-recurrence-cancellation-with-definition-revision-separation.md).
- `RecurrenceStore::history` returns one exact recurrence aggregate's complete validated lifecycle as revision-bearing typed creation and optional cancellation evidence in stream order. Creation preserves exact goal, anchor, interval, and finite count; cancellation preserves its exact caller-authored reason. A missing recurrence returns `None`. Complete decoding and canonical recurrence projection happen before any result is returned, so malformed payloads, unsupported events or versions, and impossible ordering never yield a partial prefix. The method works through read-only storage, scans no unrelated streams, mutates nothing, and grants no undo, resume, occurrence lifecycle, worker, dispatch, permission, retry, or execution authority. See [ADR-0084](adr/0084-read-only-exact-finite-recurrence-history.md).
- `RecurrenceStore::histories_page(after, page_size)` selects and fully replays at most `RecurrencePageSize + 1` authoritative finite recurrence streams after one optional caller-owned exclusive exact-ID cursor. It returns at most the requested number of `RecurrenceHistory` values in exact-ID order, each containing complete revision-bearing creation and optional cancellation evidence. `next_after` names the last returned recurrence only when validated lookahead proves another history exists; the cursor need not currently exist, and empty or beyond-end pages are terminal. Every selected history, including lookahead, is canonically validated before return, so selected corruption fails closed while corruption before the cursor, beyond lookahead, and in unrelated stream families remains isolated. Stream selection and returned-history count are bounded, while replay and aggregate allocation within each selected recurrence remain proportional to its complete lifecycle. The read-only boundary persists no cursor and grants no lifecycle or execution authority. See [ADR-0103](adr/0103-bounded-finite-recurrence-history-paging.md).
- `vela-dev recurrence history-page DATABASE PAGE_SIZE [AFTER]` validates the bounded size and optional exact-ID cursor before opening storage read-only, delegates to `histories_page`, and emits deterministic compact JSON containing complete revision-ordered recurrence histories and an optional exclusive `next_after`. Empty and beyond-end pages are terminal. Selected or lookahead corruption and unsupported future history variants fail without partial stdout; the exact and paged commands share one typed-entry conversion. The adapter creates no storage, persists no cursor, reads no clock, and grants no recurrence, occurrence, worker, permission, or execution authority. See [ADR-0104](adr/0104-read-only-bounded-finite-recurrence-history-cli-paging.md).
- `RecurrenceStore::open_read_only` opens existing recurrence evidence without creating a database or granting SQLite write authority. `list` discovers authoritative recurrence creation streams in one read snapshot, reuses exact history projection, excludes unrelated streams, and returns complete definitions ordered by exact recurrence ID. `list_by_status` reuses that complete fail-closed discovery before retaining only definitions whose exact projected status matches the caller-owned filter, preserving exact-ID order and returning an empty result for empty or unmatched inventories. Malformed owning stream IDs, payloads, events, or histories fail closed before any partial inventory is returned, including corruption whose projected status would not match the filter. These inventory boundaries read no clock, persist no occurrences or cursors, and grant no lifecycle or execution authority. See [ADR-0040](adr/0040-read-only-finite-recurrence-inventory.md) and [ADR-0089](adr/0089-read-only-finite-recurrence-status-filtering.md).
- `RecurrenceStore::list_page(after, page_size)` selects and fully replays at most `RecurrencePageSize + 1` authoritative creation streams after one optional caller-owned exclusive exact-ID cursor. Page sizes accept 1 through 1024. Results contain at most the requested number of complete definitions in exact-ID order; `next_after` is the last returned ID only when a validated lookahead proves another definition exists. The cursor need not currently exist. Empty and beyond-end pages are terminal. Every selected history, including lookahead, is canonically validated before return, so selected corruption fails closed while corruption before the cursor or after the lookahead remains outside bounded work. Unrelated streams are excluded. Paging works through read-only storage, reads no clock, mutates nothing, persists no cursor, and grants no lifecycle or execution authority. See [ADR-0091](adr/0091-bounded-finite-recurrence-inventory-paging.md).
- `RecurrenceStore::list_by_status_page(status, after, scan_size)` selects and fully replays at most `scan_size + 1` authoritative recurrence definitions after one optional caller-owned exclusive exact-ID cursor, using validated `RecurrencePageSize` values from 1 through 1024 as the inspected-inventory bound. It validates the complete selected window including lookahead before filtering the first at most `scan_size` definitions by exact persisted status. Matching definitions preserve exact-ID order. When lookahead proves more inventory exists, `next_after` identifies the last inspected recurrence even if no definition matched; otherwise it is absent. Thus sparse empty pages advance without overlapping scans, while empty stores and beyond-end cursors are terminal. Corruption in the inspected window or lookahead fails closed; corruption before the cursor, beyond lookahead, and unrelated streams remain isolated. This read-only scan persists no cursor and grants no lifecycle, occurrence, worker, permission, or execution authority. See [ADR-0097](adr/0097-bounded-sparse-finite-recurrence-status-paging.md).
...
- `RecurrenceStore::persist_due_occurrences_page(id, expected_revision, start_offset, page_size, cutoff)` validates one exact recurrence aggregate revision, reuses the bounded inclusive-cutoff selection above, and atomically appends canonical provenance for every selected coordinate. Every selected stream must be absent; an existing coordinate is strictly validated and rejected with typed `OccurrenceAlreadyPersisted` evidence rather than skipped. The recurrence prerequisite and all selected streams are rechecked in one immediate transaction, so success persists the complete page while every stale, duplicate, corruption, serialization, storage, cancellation, or competing-write failure persists none. An empty future-horizon page succeeds without writes and preserves its unchanged resumable cursor. The operation reads no ambient clock, persists no cursor, and grants no global discovery, catch-up policy, generated identity, materialization, task lifecycle, dispatch, retry, permission, or execution authority. See [ADR-0058](adr/0058-atomic-bounded-due-recurrence-provenance-persistence.md).
- `RecurrenceStore::materialize_due_occurrences_page(id, expected_revision, start_offset, page_size, cutoff, task_ids)` reuses bounded due paging and requires exactly one distinct caller-owned task ID per selected coordinate in authored-offset order. One immediate transaction rechecks the recurrence aggregate revision plus every selected occurrence and task stream, then appends each coordinate's canonical persisted and materialized events with its authoritative-goal `task.started` event. Success returns complete revision-2 bindings and the unchanged due-page cursor. Typed count mismatch, duplicate identity, existing provenance, task collision, corruption, storage failure, or contention leaves the whole page absent; a future empty page requires no IDs and writes nothing. Existing exact materialization remains the available-provenance recovery boundary. The operation reads no clock, generates no identity, persists no cursor, and grants no discovery, catch-up, claim, dispatch, retry, permission, or execution authority. See [ADR-0066](adr/0066-atomic-bounded-due-recurrence-task-materialization.md).
- `RecurrenceStore::persist_occurrence(id, expected_revision, offset)` records one exact authored coordinate as version-1 `recurrence.occurrence_persisted` evidence after exact definition replay, aggregate-revision validation, and authoritative projection. The canonical internal stream identity is a collision-free byte-length-prefixed recurrence ID plus offset; duplicates return typed evidence and never replace the first event. `load_occurrence` replays only that coordinate and requires its exact ID, definition revision, offset, goal, and instant to agree with the authoritative definition. Missing coordinates return `None`; malformed, unsupported, divergent, or invalid lifecycle histories fail closed. Cancelled recurrences reject new provenance while leaving previously authored or persisted evidence inspectable. Persistence reads no clock and grants no catch-up, schedule/task materialization, claim, dispatch, or execution authority. See [ADR-0045](adr/0045-durable-exact-recurrence-occurrence-provenance.md).
- `RecurrenceStore::occurrence_history(id, offset)` returns one exact persisted occurrence's complete validated lifecycle as revision-bearing typed persistence, claim, release, and materialization evidence in stream order. Persistence preserves the complete canonical occurrence, releases preserve exact caller-authored reasons, and materialization preserves the exact task ID. A missing occurrence stream returns `None`; an occurrence stream without its authoritative recurrence definition is invalid. Complete recurrence and occurrence decoding plus canonical lifecycle projection happen before any result is returned, so malformed payloads, unsupported events or versions, divergent provenance, and impossible ordering never yield a partial prefix. Aggregate cancellation does not hide existing history. The method works through read-only storage, scans no unrelated coordinates, mutates nothing, and grants no discovery, lifecycle, worker, dispatch, retry, permission, or execution authority. See [ADR-0085](adr/0085-read-only-exact-recurrence-occurrence-history.md).
- `RecurrenceStore::occurrence_histories_page(id, start_offset, page_size)` strictly loads one exact recurrence definition and inspects at most one validated authored-offset window. It returns each present coordinate's complete revision-bearing occurrence history in increasing offset order and omits missing streams, including valid all-gap pages. `next_offset` advances by inspected authored coordinates or reports finite completion, independently of result density. The selected definition is replayed once and every present selected stream reuses exact canonical lifecycle projection, so malformed, unsupported, divergent, or impossible selected evidence fails closed before partial output while unrelated and out-of-window corruption remains isolated. The page works through read-only storage, reads no clock, mutates nothing, persists no cursor, and grants no global discovery, lifecycle, worker, lease, dispatch, retry, permission, or execution authority. See [ADR-0087](adr/0087-bounded-recurrence-occurrence-history-paging.md).
- `RecurrenceStore::claim_occurrence(id, offset, expected_occurrence_revision, cutoff)` durably reserves one exact available coordinate by appending version-1 `recurrence.occurrence_claimed` at its exact observed revision only when its authoritative instant is at or before the inclusive caller-owned cutoff. The same transaction rechecks the observed recurrence aggregate revision so a cancellation that commits first blocks the claim. Success returns complete `ClaimedRecurrenceOccurrence` provenance; `load_claimed_occurrence` exposes only the current exact reservation after reopen. Revision validation precedes lifecycle and due checks, and racing claims commit exactly one event. Existing persisted provenance views include claimed coordinates, while materialized views exclude them and direct exact materialization rejects current claims. The claim reads no clock and grants no worker identity, lease, permission, dispatch, retry, or execution authority. See [ADR-0068](adr/0068-exact-persisted-recurrence-occurrence-claiming.md).
...
- `RecurrenceStore::claimed_occurrences_page(id, start_offset, page_size)` reuses one exact recurrence definition and the existing bounded authored-offset window, but returns only complete `ClaimedRecurrenceOccurrence` values whose strictly replayed current lifecycle state remains claimed. Missing, persisted-only, released, and materialized coordinates are omitted, including valid all-gap pages, while `next_offset` advances by inspected authored coordinates and truncates at the finite end. Selected-window corruption fails closed before partial output; unrelated recurrences and out-of-window coordinates cannot block inspection. The page works through read-only storage, mutates nothing, persists no cursor, and grants no global inventory, claim-next, worker, lease, dispatch, retry, permission, or execution authority. See [ADR-0074](adr/0074-bounded-claimed-recurrence-occurrence-paging.md).
- `RecurrenceStore::available_occurrences_page(id, start_offset, page_size)` reuses one exact recurrence definition and the existing bounded authored-offset window, but returns only complete `AvailableRecurrenceOccurrence` values whose strictly replayed current lifecycle state is persisted-only or explicitly released. Every result preserves its exact current revision and optional latest caller-authored release evidence. Missing, claimed, and materialized coordinates are omitted, including valid all-gap pages, while `next_offset` advances by inspected authored coordinates and truncates at the finite end. Selected-window corruption fails closed before partial output; unrelated recurrences and out-of-window coordinates cannot block inspection. The page works through read-only storage, reads no clock, mutates nothing, persists no cursor, and grants no global inventory, claim-next, worker, lease, dispatch, retry, permission, or execution authority. See [ADR-0076](adr/0076-bounded-available-recurrence-occurrence-paging.md).
- `RecurrenceStore::claim_next_available_occurrence(id, start_offset, page_size, cutoff)` inspects one exact recurrence and at most one validated authored-offset window, then claims the earliest currently available coordinate at or before the inclusive caller-owned cutoff. Missing, claimed, and materialized coordinates are skipped; persisted-only and released coordinates use their exact current revisions. Success returns complete claim evidence and the following authored cursor or finite completion. An all-gap window advances to its first uninspected coordinate, while a future first-available coordinate returns no claim and preserves that offset for a later cutoff. A persisted competing transition restarts the same bounded selection; selected-window corruption fails closed, and unrelated or out-of-window evidence cannot block selection. The operation reads no clock, persists no cursor, performs no cross-recurrence discovery, and grants no worker, lease, dispatch, retry, permission, or execution authority. See [ADR-0078](adr/0078-bounded-next-available-recurrence-occurrence-claiming.md).
- `RecurrenceStore::materialize_next_available_occurrence(id, start_offset, page_size, cutoff, task_id)` reuses the exact bounded availability selection and atomically binds its earliest due coordinate to one caller-owned inert task. Missing, claimed, and materialized coordinates are skipped; persisted-only and released coordinates use their exact current revisions. Success returns the complete materialized binding and following authored cursor or finite completion. An all-gap window advances, while a future first-available coordinate writes nothing and preserves its offset. One transaction rechecks the recurrence aggregate revision and complete selected window before appending the occurrence materialization and authoritative-goal task start. Persisted competing lifecycle transitions restart bounded selection and fail with typed exhaustion after four conflicts; task collisions, replay, storage, read-only, and append failures create no partial binding. The operation reads no clock, persists no cursor, generates no identity, performs no cross-recurrence discovery, and grants no worker, lease, dispatch, retry, permission, or execution authority. See [ADR-0080](adr/0080-bounded-next-available-recurrence-task-materialization.md).
...
- `RecurrenceStore::materialized_occurrences_global_page(after, page_size)` selects at most one validated `OccurrencePageSize + 1` window of streams carrying authoritative occurrence-materialization markers and returns complete bindings in deterministic canonical stream-key order. Its typed opaque cursor preserves only the exact recurrence ID and authored offset; callers must not infer recurrence-ID or numeric-offset order because the internal collision-free key is ordered lexically. Every selected occurrence lifecycle and authoritative recurrence definition, including lookahead, is replayed and canonically validated before return. `next_after` names the last emitted coordinate only when another validated binding exists. Evidence at or before the exclusive cursor, beyond lookahead, and in unrelated stream families remains isolated. The page does not establish task-identity uniqueness outside its window, validate task lifecycle, expose raw stream keys, persist cursors, mutate state, read a clock, or grant worker, dispatch, retry, permission, or execution authority. See [ADR-0105](adr/0105-bounded-global-materialized-recurrence-binding-paging.md).
- `vela-dev recurrence materialized-page DATABASE PAGE_SIZE [--after-recurrence-id ID --after-offset OFFSET]` validates the bounded size and the all-or-none typed cursor coordinate before opening storage read-only, delegates to `materialized_occurrences_global_page`, and emits compact JSON containing complete bindings plus `next_after` as either `null` or an exact recurrence-ID and authored-offset pair. The cursor may be round-tripped but exposes no internal stream key or ordering semantics. Missing storage remains missing; selected or lookahead corruption, open, replay, provenance, and serialization failures return non-zero without partial stdout. The adapter persists no cursor, claims no task-identity uniqueness across pages, reads no clock, mutates nothing, and grants no lifecycle, worker, lease, dispatch, retry, permission, or execution authority. See [ADR-0106](adr/0106-read-only-global-materialized-recurrence-binding-cli-paging.md).
- `RecurrenceStore::materialize_occurrence(id, offset, expected_occurrence_revision, task_id)` requires one exact strictly validated available coordinate, either persisted-only at revision 1 or explicitly released at its later exact revision. It atomically rechecks the observed recurrence aggregate revision, appends `recurrence.occurrence_materialized` at the exact occurrence revision, and appends the existing `task.started` event with `ExpectedVersion::NoStream`; a cancellation that commits first blocks both writes, and the task receives the occurrence's authoritative goal. The resulting materialized projection preserves the exact occurrence, resulting occurrence revision, and caller-owned task ID. Missing provenance, stale revisions, current claimed state, task collisions, duplicate or replacement bindings, malformed lifecycle histories, and competing transitions leave both streams unchanged. Existing `load_occurrence` and sparse persisted paging remain provenance views over every valid lifecycle; claimed, released, and materialized exact lookups expose only their respective current states. The boundary reads no clock, generates no identity, scans no unrelated coordinates, and grants no catch-up, selection, dispatch, retry, permission, or execution authority. See [ADR-0050](adr/0050-atomic-exact-recurrence-occurrence-task-materialization.md) and [ADR-0069](adr/0069-exact-recurrence-occurrence-claim-release.md).
...
- `ScheduleStore::histories_page(after, page_size)` selects and fully replays at most `SchedulePageSize + 1` authoritative one-shot schedule streams after one optional caller-owned exclusive exact-ID cursor. It returns at most the requested number of `ScheduleHistory` values in exact-ID order, each containing complete revision-bearing creation, claim, release, cancellation, and materialization evidence. `next_after` names the last returned schedule only when validated lookahead proves another history exists; the cursor need not exist, and empty or beyond-end pages are terminal. Every selected history, including lookahead, is canonically validated before return, so selected corruption fails closed while corruption before the cursor, beyond lookahead, and in unrelated stream families remains isolated. Stream selection and returned-history count are bounded, while replay and aggregate allocation within each selected schedule remain proportional to its complete lifecycle. The read-only boundary persists no cursor and grants no lifecycle or execution authority. See [ADR-0101](adr/0101-bounded-one-shot-schedule-history-paging.md).
- `vela-dev schedule histories DATABASE PAGE_SIZE [AFTER]` validates the bounded size and optional exact-ID cursor before opening storage read-only, delegates to `histories_page`, and emits deterministic compact JSON containing complete revision-ordered histories and an optional exclusive `next_after`. Empty and beyond-end pages are terminal. Selected or lookahead corruption and unsupported future history variants fail without partial stdout; the exact and paged commands share one typed-entry conversion. The adapter never creates storage, persists cursors, reads ambient time, pages within one lifecycle, mutates, dispatches, grants permission, or executes work. See [ADR-0102](adr/0102-read-only-bounded-one-shot-schedule-history-cli-paging.md).
...
- `ScheduleStore::list_page(after, page_size)` selects and fully replays at most `SchedulePageSize + 1` authoritative creation streams after one optional caller-owned exclusive exact-ID cursor. Page sizes accept 1 through 1024. Results contain at most the requested number of complete schedules in exact-ID order; `next_after` is the last returned ID only when a validated lookahead proves another schedule exists. The cursor need not currently exist. Empty and beyond-end pages are terminal. Every selected history, including lookahead, is canonically validated before return, so selected corruption fails closed while corruption before the cursor or after the lookahead remains outside bounded work. Unrelated streams are excluded. Paging works through read-only storage, reads no clock, mutates nothing, persists no cursor, and grants no lifecycle or execution authority. See [ADR-0093](adr/0093-bounded-one-shot-schedule-inventory-paging.md).
...
- `ScheduleStore::list_by_status_page(status, after, scan_size)` strictly projects at most one validated `SchedulePageSize + 1` exact-ID-ordered creation-stream window, filters the first `scan_size` projections by exact persisted status, and returns a sparse `ScheduleStatusPage`. When validated lookahead proves more inventory exists, `next_after` is the last **inspected** ID rather than the last emitted ID, so all-nonmatching windows still advance without unbounded scanning. Empty, beyond-end, and terminal windows have no cursor. Corruption in the inspected window or lookahead fails closed; corruption before the cursor, beyond lookahead, and in unrelated stream families is isolated. The operation is read-only, reads no ambient clock, and grants no mutation or execution authority. See [ADR-0095](adr/0095-bounded-sparse-one-shot-schedule-status-paging.md).
- `ScheduleStore::list_due_page(cutoff, after, scan_size)` selects and fully replays at most `scan_size + 1` authoritative schedules in exact-ID order after one optional caller-owned exclusive ID cursor, using validated `SchedulePageSize` values from 1 through 1024 as the inspected-inventory bound. It filters the first at most `scan_size` complete projections to exactly pending schedules due at or before the inclusive caller-owned cutoff. Matches preserve exact-ID scan order rather than complete `list_due`'s global due-instant order. When lookahead proves more inventory exists, `next_after` identifies the last inspected schedule even if no schedule matched; otherwise it is absent. Corruption in the inspected window or lookahead fails closed while corruption before the cursor, beyond lookahead, and in unrelated streams remains isolated. The read-only scan reads no ambient clock, persists no cursor, and grants no lifecycle or execution authority. See [ADR-0099](adr/0099-bounded-sparse-one-shot-schedule-due-paging.md).
...
`occurrence_count`, lowercase `status`, `final_occurrence_unix_millis`,
immutable `definition_revision`, mutable `aggregate_revision`, and nullable
`cancellation`. Creation emits `active` status, definition and aggregate
revision `1`, and null cancellation. Invalid IDs, goals, intervals, and counts emit
`invalid_recurrence_id`, `invalid_task_goal`, `invalid_recurrence_interval`, or
`invalid_occurrence_count` without creating storage. Duplicate, overflow, open,
schema, append, and serialization failures emit one escaped
`recurrence_creation_failed` diagnostic and no stdout; failed creation cannot
replace an existing definition or persist an overflowing one.
...
Cancelled recurrences remain inspectable through exact lookup. Their projection
still exposes the immutable authored definition, while lowercase `status`,
`aggregate_revision`, and `cancellation` truthfully report the later aggregate
state from
...
recurrence ID before storage access; clap parses the observed aggregate
...
recurrences, invalid starts, duplicates, cancellation, selected corruption,
...
identities before storage access; clap parses the observed aggregate revision,
...
storage access. Missing or stale recurrences, invalid starts, cancellation,
existing or malformed selected provenance, task collisions, concurrency, open, replay,
...
aggregate revision, authored offset, and caller-owned cutoff as non-negative
`u64` values. It opens only the selected database through
...
Invalid IDs and page sizes emit `invalid_recurrence_id` and
`invalid_occurrence_page_size` without accessing storage. Missing recurrences,
stale revisions, out-of-range starts, duplicates, cancellation, selected corruption,
concurrency, open, persistence, and serialization failures emit
...
Missing or stale recurrences, invalid starts, count mismatch, duplicate task
IDs, cancellation, selected provenance or corruption, task collisions, concurrency, open,
...
aggregate revision and zero-based offset as non-negative `u64` values before
...
revisions, cancellation, out-of-range offsets, duplicate coordinates, invalid durable history,
...
## Read-only claimed recurrence occurrence CLI paging

`vela-dev recurrence claimed DATABASE RECURRENCE_ID START_OFFSET PAGE_SIZE`
validates the exact recurrence ID and positive, at-most-1024 page size before
storage access. It opens only the selected existing database through
`RecurrenceStore::open_read_only` and delegates the bounded authored-offset
window to `RecurrenceStore::claimed_occurrences_page`.

Success emits complete current claims in ascending offset order. Each object
preserves exact `recurrence_id`, `goal`, `offset`, `unix_millis`,
`definition_revision`, and `occurrence_revision`. Missing, persisted-only,
released, and materialized coordinates are omitted; `next_offset` still
advances by every inspected authored coordinate and is `null` at the finite
end.
...
`claimed_recurrence_occurrence_lookup_failed` with non-zero status and no
...
global inventory, claim-next selection, generated identity, worker identity,
lease, dispatch, workflow, provider/tool, permission, retry, or execution
authority. See
[ADR-0075](adr/0075-read-only-claimed-recurrence-occurrence-cli-paging.md).

## Read-only available recurrence occurrence CLI paging

`vela-dev recurrence available DATABASE RECURRENCE_ID START_OFFSET PAGE_SIZE`
validates the exact recurrence ID and positive, at-most-1024 page size before
storage access. It opens only the selected existing database through
`RecurrenceStore::open_read_only` and delegates the bounded authored-offset
window to `RecurrenceStore::available_occurrences_page`.

Success emits complete current available coordinates in ascending offset order.
Each object preserves exact `recurrence_id`, `goal`, `offset`, `unix_millis`,
`definition_revision`, `occurrence_revision`, and `latest_release`.
Persisted-only coordinates emit a null release; released coordinates preserve
the exact latest caller-authored recovery reason. Missing, claimed, and
materialized coordinates are omitted; `next_offset` still advances by every
inspected authored coordinate and is `null` at the finite end.
...
`recurrence_occurrence_out_of_range`. Open, strict selected-window replay,
provenance, paging, and serialization failures emit
`available_recurrence_occurrence_lookup_failed` with non-zero status and no
stdout. Missing storage remains missing, and unrelated or out-of-window
corruption cannot block the selected page.
...
`interval_millis`, `occurrence_count`, lowercase `status`,
`final_occurrence_unix_millis`, `definition_revision`, `aggregate_revision`,
and nullable `cancellation`. An empty compatible store emits
`{"recurrences":[]}`; exact strings are JSON
escaped.
