...
- `ScheduleStore::materialize_next_due(cutoff, task_id)` selects the earliest pending due intent in the same order and atomically appends its `schedule.materialized` event with the existing `task.started` event for the caller-owned task identity. It returns `None` when no work is due. A competing schedule transition restarts selection; a task-ID collision consumes no schedule. Direct pending materialization and explicit claimed materialization are both valid histories. The operation generates no identity, reads no clock, grants no permission, and dispatches or executes no work.
