...
The append guards both the exact workflow revision and the task stream version whose evidence was evaluated. A task change reloads and re-evaluates the gate, so newer failed evidence blocks stale advancement. A workflow change remains authoritative and never causes a phase-relative transition index to be retried against new state. The low-level `advance` acknowledgement operation remains available, and the evidence-backed boundary does not choose a transition, run a verifier/provider/tool/command, grant permission, mutate the task, or synchronize terminal lifecycles.

The opt-in terminal synchronization boundary is specified by [ADR-0033](adr/0033-verified-terminal-workflow-task-synchronization.md). `WorkflowRunStore::advance_and_complete_task_if_verification_passes` accepts the same exact run, transition, and Attempt authority plus a validated task output, and additionally requires the authored transition to target a terminal phase and the attributed task to remain active. After the authored gate passes, it appends the existing advancement with the exact gate acknowledgement and the existing task completion with the exact output in one two-stream transaction. A task evidence race is re-evaluated, a workflow revision race remains authoritative, and no rejection can persist only one of the requested lifecycle events. Independent raw, workflow-only, and task-only operations remain unchanged; terminal synchronization does not execute a verifier or choose a transition.
