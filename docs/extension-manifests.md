...
The task-evidence bridge is specified by [ADR-0021](adr/0021-workflow-phase-task-attempt-evidence.md). `AssistantRuntime::execute_workflow_phase_task_turn` requires an exact active task already associated with a writable session, a fresh Attempt observation ID, and the same explicit phase, registry, human content, and policy inputs as the session-only operation. Before transcript or provider effects, it validates the task association and lifecycle, Attempt identity, session writability, and phase bindings. A successful response is committed atomically as both the assistant transcript turn and the task's exact Attempt text.

Provider failure or a blank-only provider response preserves the committed human turn and appends neither an assistant turn nor an Attempt. After response validation, the assistant turn and Attempt append in one SQLite transaction, so a racing task or session rejection leaves neither response record. The Attempt is response evidence only: the operation does not accept or infer a workflow-run ID, prove phase provenance, persist phase or skill-selection identity, synchronize task and workflow lifecycles, infer success, complete either aggregate, choose or apply a transition, schedule work, grant tools, or invoke a tool.

## Evidence-backed durable workflow advancement

The explicit task-Verification bridge is specified by [ADR-0032](adr/0032-task-verification-workflow-advancement.md). `WorkflowRunStore::advance_if_task_verification_passes` requires one task-attributed durable run, exact caller-owned run revision, caller-selected transition index, and exact task Attempt. The selected transition must declare a gate; its exact authored gate ID becomes the single task Verification check that must be identified. Only the latest matching structured Verification for that Attempt can authorize the existing advancement event and exact gate acknowledgement. Pending and failed reports remain inspectable, while unattributed or ungated runs and invalid Attempt lineage fail without writing.

The append guards both the exact workflow revision and the task stream version whose evidence was evaluated. A task change reloads and re-evaluates the gate, so newer failed evidence blocks stale advancement. This re-evaluation is bounded to four append attempts; continuous task conflicts then return `WorkflowVerifiedAdvanceError::TaskConflictRetriesExhausted` without advancing the workflow. A workflow change remains authoritative and never causes a phase-relative transition index to be retried against new state. The low-level `advance` acknowledgement operation remains available, and the evidence-backed boundary does not choose a transition, run a verifier/provider/tool/command, grant permission, mutate the task, or synchronize terminal lifecycles.

The opt-in terminal synchronization boundary is specified by [ADR-0033](adr/0033-verified-terminal-workflow-task-synchronization.md). `WorkflowRunStore::advance_and_complete_task_if_verification_passes` accepts the same exact run, transition, and Attempt authority plus a validated task output, and additionally requires the authored transition to target a terminal phase and the attributed task to remain active. After the authored gate passes, it appends the existing advancement with the exact gate acknowledgement and the existing task completion with the exact output in one two-stream transaction. A task evidence race is re-evaluated within the same four-attempt bound, a workflow revision race remains authoritative, and exhaustion or any other rejection persists neither requested lifecycle event. Independent raw, workflow-only, and task-only operations remain unchanged; terminal synchronization does not execute a verifier or choose a transition.
