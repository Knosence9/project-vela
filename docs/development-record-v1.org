...
Sampling emits no partial stdout. An unreadable corpus root exits with status =2=. An unreadable record, malformed JSON, semantic validation failure, or non-UTF-8 relative record path exits with status =1= after deterministic path-prefixed diagnostics. Discovery neither follows symbolic links nor treats non-regular =.json= entries as records; descriptor-anchored checks reject a regular-file candidate replaced before reading. Sampling is read-only and performs no scoring, indexing, network access, or mutation.
