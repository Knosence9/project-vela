...
The required limit is from =1= through =1024=. The optional filter accepts exactly =untrusted=, =reviewed=, or =curated=. Success emits one compact JSON array containing each selected relative =path= and complete =record=; an empty match emits =[]=. Selection uses sorted relative-path order and never reads randomness or ambient state.
