...
:PROPERTIES:
:CUSTOM_ID: adr-0030-evaluate-required-task-verification-gates-from-durable-evidence
:END:
- *Status:* accepted
- *Date:* 2026-07-30
- *Decision and execution issue:* [[https://github.com/Knosence9/project-vela/issues/773][#773]]
...
:PROPERTIES:
:CUSTOM_ID: let-every-caller-scan-observations
:END:
...
:PROPERTIES:
:CUSTOM_ID: treat-any-historical-pass-as-sufficient
:END:
...
:PROPERTIES:
:CUSTOM_ID: persist-the-required-gate-set-on-the-task
:END:
...
:PROPERTIES:
:CUSTOM_ID: complete-tasks-automatically-when-all-gates-pass
:END:
...
:END:
[[file:0031-verification-gated-task-completion.org][ADR-0031]] subsequently composes this pure report with an explicit caller-selected completion boundary that re-evaluates on concurrent task changes; gate evaluation itself remains read-only and caller-owned. Reconsider this decision before persisting or replacing gate policy, attaching commands or artifacts, deriving workflow lifecycle transitions, requiring freshness beyond append order, adding conditional/optional gates, or introducing actors, timestamps, scheduling, retries, permissions, credentials, or remote execution.
