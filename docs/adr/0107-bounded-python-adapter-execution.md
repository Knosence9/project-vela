...
- A descendant may outlive the direct child, but inherited pipes cannot extend this call beyond its deadline.

## Verification

Strict RED→GREEN tests cover direct-child timeout and reap behavior, stdout and stderr overflow, zero-limit rejection, successful in-budget execution, CLI timeout selection, and pre-launch CLI validation. The complete repository quality gate must remain green.
