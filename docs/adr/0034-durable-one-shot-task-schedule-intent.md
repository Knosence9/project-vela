...
`ScheduleStore::materialize_next_due(cutoff, task_id)` provides a separate atomic consumption boundary for callers that already own the task identity and do not need a recoverable claim gap. It selects the earliest pending due schedule in the same deterministic order. It atomically appends `schedule.materialized` against the exact pending revision and a new `TaskEvent::Started` with `ExpectedVersion::NoStream`; success returns `Some(ScheduledTask)` containing the updated schedule revision and exact task binding. A competing schedule transition restarts selection; a task-ID collision consumes no schedule and is returned without generating another identity. No due work returns `None`. Direct pending materialization and the existing explicit `claimed -> materialized` path are both valid histories. The operation remains inert: it reads no clock and dispatches or executes no work.

`vela-dev schedule materialize-next DATABASE CUTOFF_UNIX_MILLIS TASK_ID` exposes that atomic boundary without duplicating selection policy. It validates the caller-owned task identity before opening storage, supplies the caller-owned cutoff, and emits the complete materialized projection under a `schedule` key or `null` when no work is due. Invalid identity, task collision, storage, replay, and serialization failures emit no partial JSON and cannot leave one side of the schedule/task transition committed. The CLI gains no ambient clock, identity generation, dispatch, workflow, provider, tool, permission, retry, or execution authority.
