...
`SessionStoreError` is non-exhaustive. Wrapped event-log and replay failures are exposed through `std::error::Error::source`; domain errors such as `AlreadyExists`, `NotFound`, `AlreadyClosed`, `AlreadyOpen`, `InvalidStreamId`, and `InvalidHistory` have no source.

## Non-goals

This slice does not add automatic or model-generated summaries, structured close- or reopen-reason taxonomies, a duplicate mutable session index or session-side task membership index, listing filters, pagination, moving or detaching tasks, messages, turns, branching, compression, timestamps, actors, metadata, a runtime interface, or async execution. Those require separate lifecycle events and acceptance tests rather than assumptions in persisted state.
