...
- Newly created sessions expose neither a close nor reopen reason. Reopened sessions expose no active close reason and expose the reason from their latest reopen event. Closed sessions expose no active reopen reason and expose the reason from their latest close event.
...
- Creating an existing ID returns `SessionStoreError::AlreadyExists` and leaves the original history unchanged. The event log's expected-version transaction also enforces create, rename, close, and reopen writes under racing writers.
- Unknown event discriminators or payload versions and malformed payloads, including an empty persisted title in creation or rename events, payload-version-2 close reason, or payload-version-2 reopen reason, remain explicit `ReplayError` values wrapped by `SessionStoreError::Replay`; persisted data is never skipped.
- Valid histories start with one `session.created` event, allow title changes after creation, and otherwise alternate `session.closed` and `session.reopened`. Rename, close, or reopen without creation, duplicate creation, duplicate close, duplicate reopen, and any other ordering are invalid history rather than implicit state changes.
