...
- `EventLogError::VersionGap` reports the first expected and observed stored versions when append encounters a stream that does not start at `1` or contains an internal gap; the event is not encoded and no row is written.
