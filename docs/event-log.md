...
- `EventLog::open_read_only` opens only an existing SQLite file with SQLite read-only flags, enables connection-local `query_only` defense in depth, verifies WAL without changing journal or synchronous settings, and validates the required event columns and compound primary key without creating schema. A missing file is a storage error; a missing or structurally incompatible `events` table returns `IncompatibleEventSchema`. Replay remains available, while append attempts fail as storage errors and persist nothing.
- `EventLog::append_if_stream_unchanged` additionally guards an append with the expected version of a second stream. Both expectations are rechecked in the same immediate SQLite transaction; if either stream changed, `WrongExpectedVersion` is returned and no row is written. This supports small cross-aggregate invariants without batch appends.
