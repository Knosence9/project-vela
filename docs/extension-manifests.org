...
The accepted first execution boundary is specified by [[file:adr/0003-tools-only-wasm-component-boundary.org][ADR-0003]]. It limits activation to kind-constrained tool selections and no-import WebAssembly components. Selection remains inert until a caller explicitly invokes the separate activation operation below.

** Tool artifact preparation
:PROPERTIES:
:CUSTOM_ID: tool-artifact-preparation
:END:
...
:PROPERTIES:
:CUSTOM_ID: declarative-workflow-definition-preparation
:END:
The workflow preparation contract is specified by [[file:adr/0006-inert-declarative-workflow-definitions.org][ADR-0006]] and extended with inert phase skill bindings by [[file:adr/0018-inert-workflow-phase-skill-bindings.org][ADR-0018]]. =prepare_workflow_definitions(root, selection)= accepts only selected =Workflow= records, rejects the lexicographically first other kind before filesystem access, and treats an empty selection as a filesystem-free success. A non-empty selection reuses descriptor-anchored root, package, manifest, and entrypoint revalidation before reading at most 1 MiB plus one probe byte.
...
:PROPERTIES:
:CUSTOM_ID: atomic-inert-workflow-registration
:END:
The accepted registration boundary is specified by [[file:adr/0007-caller-owned-inert-workflow-registration.org][ADR-0007]]. The kernel =WorkflowRegistry= is a caller-owned, process-local exact-ID directory of immutable owned workflow topology. Atomic batch registration rejects the lexicographically first collision against either the existing registry or the batch without mutation. Exact lookup preserves identity, registry enumeration uses ascending exact-ID order, and phases and transitions retain authored order internally.
...
:PROPERTIES:
:CUSTOM_ID: explicit-workflow-phase-provider-composition
:END:
The tool-free composition bridge is specified by [[file:adr/0020-explicit-workflow-phase-provider-composition.org][ADR-0020]]. =AssistantRuntime::execute_workflow_phase_turn= requires the caller to supply one exact borrowed phase together with the session, human content, system policy, developer policy, and caller-owned skill registry. It resolves the phase through the boundary above before any transcript or provider side effect, then reuses the existing composed-turn authority structure. The provider receives system policy, developer policy, phase-bound registered skills in deterministic ascending exact-ID order, and the durable transcript as distinct fields. Registered but unbound skills are excluded.
...
:PROPERTIES:
:CUSTOM_ID: evidence-backed-durable-workflow-advancement
:END:
The explicit task-Verification bridge is specified by [[file:adr/0032-task-verification-workflow-advancement.org][ADR-0032]]. =WorkflowRunStore::advance_if_task_verification_passes= requires one task-attributed durable run, exact caller-owned run revision, caller-selected transition index, and exact task Attempt. The selected transition must declare a gate; its exact authored gate ID becomes the single task Verification check that must be identified. Only the latest matching structured Verification for that Attempt can authorize the existing advancement event and exact gate acknowledgement. Pending and failed reports remain inspectable, while unattributed or ungated runs and invalid Attempt lineage fail without writing.
...
:PROPERTIES:
:CUSTOM_ID: tool-component-invocation
:END:
...
:PROPERTIES:
:CUSTOM_ID: atomic-tool-activation
:END:
...
:PROPERTIES:
:CUSTOM_ID: atomic-tool-deactivation
:END:
...
:PROPERTIES:
:CUSTOM_ID: atomic-active-tool-replacement
:END:
...
:PROPERTIES:
:CUSTOM_ID: atomic-active-tool-reconciliation
:END:
...
:PROPERTIES:
:CUSTOM_ID: developer-cli-inspection
:END:
...
:PROPERTIES:
:CUSTOM_ID: developer-cli-invocation
:END:
...
:PROPERTIES:
:CUSTOM_ID: ownership-and-trust-boundary
:END:
