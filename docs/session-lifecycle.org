...
:PROPERTIES:
:CUSTOM_ID: persisted-session-lifecycle
:END:
...
The separate [[file:assistant-runtime.org][=single-turn assistant runtime=]] composes this transcript boundary with a caller-supplied provider without changing session events.

See [[file:event-log.org][=event-log.org=]] for the underlying append, durability, concurrency, and replay guarantees.
