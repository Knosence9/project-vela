...
#[test]
fn recurrence_history_preserves_exact_authored_and_cancellation_evidence() {
    let directory = tempdir().unwrap();
    let path = directory.path().join("events.sqlite3");
    let id = RecurrenceId::new("history").unwrap();
    let missing = RecurrenceId::new("missing-history").unwrap();
    let goal = TaskGoal::new("Preserve exact recurrence history").unwrap();
    let interval = ScheduleInterval::from_millis(7).unwrap();
    let count = OccurrenceCount::new(3).unwrap();
    let reason = RecurrenceCancellation::new(" exact withdrawal ").unwrap();
    let mut store = RecurrenceStore::open(&path).unwrap();
    store
        .create(id.clone(), goal.clone(), instant(5), interval, count)
        .unwrap();
    ...
    assert_eq!(
        ...
        [
            ...
            (2, RecurrenceHistoryEvent::Cancelled { reason }),
        ]
    );
...
#[test]
fn pages_complete_recurrence_histories_in_exact_id_order() {
    let empty_directory = tempdir().unwrap();
    let empty_path = empty_directory.path().join("events.sqlite3");
    drop(RecurrenceStore::open(&empty_path).unwrap());
    let empty = RecurrenceStore::open_read_only(&empty_path)
        .unwrap()
        .histories_page(None, RecurrencePageSize::new(2).unwrap())
        .unwrap();
    assert!(empty.histories().is_empty());
    ...
    let bravo = RecurrenceId::new("bravo").unwrap();
    store
        .cancel(
            &bravo,
            1,
            RecurrenceCancellation::new(" exact withdrawal ").unwrap(),
        )
        .unwrap();
    drop(store);

    let store = RecurrenceStore::open_read_only(&path).unwrap();
    let page_size = RecurrencePageSize::new(2).unwrap();
    let first = store.histories_page(None, page_size).unwrap();
    assert_eq!(
        first
            .histories()
            .iter()
    ...
    assert!(matches!(
        first.histories()[1].entries()[1].event(),
        RecurrenceHistoryEvent::Cancelled { reason }
            if reason.as_str() == " exact withdrawal "
    ));

    let second = store.histories_page(first.next_after(), page_size).unwrap();
    assert_eq!(
        second
            .histories()
            .iter()
    ...
    let from_nonexistent = store.histories_page(Some(&between), page_size).unwrap();
    assert_eq!(from_nonexistent.histories()[0].id().as_str(), "delta");
    assert_eq!(from_nonexistent.next_after(), None);

    let beyond = RecurrenceId::new("zzzz").unwrap();
    let terminal = store.histories_page(Some(&beyond), page_size).unwrap();
    assert!(terminal.histories().is_empty());
    assert_eq!(terminal.next_after(), None);
}

#[test]
fn recurrence_history_pages_fail_closed_only_for_the_bounded_window() {
    let directory = tempdir().unwrap();
    let path = directory.path().join("events.sqlite3");
    let mut store = RecurrenceStore::open(&path).unwrap();
    for id in ["alpha", "bravo", "charlie", "delta"] {
    ...
    let store = RecurrenceStore::open_read_only(&path).unwrap();
    let page = store
        .histories_page(Some(&after), RecurrencePageSize::new(1).unwrap())
        .unwrap();
    assert_eq!(page.histories()[0].id().as_str(), "bravo");
...
#[test]
fn recurrence_history_fails_closed_for_selected_invalid_lifecycle_only() {
    let directory = tempdir().unwrap();
    let path = directory.path().join("events.sqlite3");
    let id = RecurrenceId::new("invalid-history").unwrap();
    let unrelated = RecurrenceId::new("unrelated-invalid-history").unwrap();
    let mut store = RecurrenceStore::open(&path).unwrap();
    store
        .create(
            id.clone(),
            TaskGoal::new("Reject partial history").unwrap(),
            instant(1),
    ...
    store
        ...
        .unwrap();
    insert_recurrence_cancellation(&path, &unrelated, 2, "first cancellation");
    insert_recurrence_cancellation(&path, &unrelated, 3, "duplicate cancellation");

    assert_eq!(store.history(&id).unwrap().unwrap().len(), 1);
    assert!(matches!(
        store.history(&unrelated).unwrap_err(),
        RecurrenceStoreError::InvalidHistory { event_count: 3 }
    ));
}

#[test]
fn recurrence_history_rejects_selected_decode_failure_without_scanning_unrelated_streams() {
    let directory = tempdir().unwrap();
    let path = directory.path().join("events.sqlite3");
    let selected = RecurrenceId::new("selected-history").unwrap();
    let unrelated = RecurrenceId::new("unrelated-history").unwrap();
    let mut store = RecurrenceStore::open(&path).unwrap();
    for id in [&selected, &unrelated] {
        store
            .create(
                id.clone(),
                TaskGoal::new(format!("history for {id}")).unwrap(),
                instant(1),
                ScheduleInterval::from_millis(1).unwrap(),
                OccurrenceCount::new(1).unwrap(),
            )
    ...
    connection
        .execute(
            ...
            [format!("recurrence:{unrelated}")],
        )
        .unwrap();

    assert_eq!(store.history(&selected).unwrap().unwrap().len(), 1);

    connection
        .execute(
            "UPDATE events SET event_type = 'recurrence.unknown'
             WHERE stream_id = ?1 AND stream_version = 1",
            [format!("recurrence:{selected}")],
        )
        .unwrap();
    assert!(matches!(
        store.history(&selected).unwrap_err(),
        RecurrenceStoreError::Replay(ReplayError::UnsupportedEvent { .. })
    ));

    connection
        .execute(
            "UPDATE events SET event_type = 'recurrence.fixed_interval_created', payload_version = 99
             WHERE stream_id = ?1 AND stream_version = 1",
            [format!("recurrence:{selected}")],
        )
        .unwrap();
    ...
    connection
        .execute(
            "UPDATE events SET payload_version = 1, payload = X'7B7D'
             WHERE stream_id = ?1 AND stream_version = 1",
            [format!("recurrence:{selected}")],
        )
        .unwrap();
...
#[test]
fn materializes_next_available_due_occurrence_with_bounded_resumable_selection() {
    let directory = tempdir().unwrap();
    let path = directory.path().join("events.sqlite3");
    let id = RecurrenceId::new("materialize-next-available").unwrap();
    let mut store = RecurrenceStore::open(&path).unwrap();
    store
        .create(
            id.clone(),
            TaskGoal::new("Materialize bounded recurrence work").unwrap(),
            instant(10),
            ScheduleInterval::from_millis(2).unwrap(),
            OccurrenceCount::new(7).unwrap(),
        )
    ...
    let selected_task = TaskId::new("selected-task").unwrap();
    let selected = store
        .materialize_next_available_occurrence(
            &id,
            0,
            OccurrencePageSize::new(5).unwrap(),
            instant(18),
            selected_task.clone(),
        )
        .unwrap();
    let materialized = selected.occurrence().unwrap();
    assert_eq!(materialized.occurrence().offset(), 1);
    assert_eq!(materialized.revision(), 4);
    assert_eq!(materialized.task_id(), &selected_task);
    assert_eq!(selected.next_offset(), Some(2));
    assert!(
        TaskStore::open(&path)
            .unwrap()
    ...
    let next = store
        .materialize_next_available_occurrence(
            ...
            OccurrencePageSize::new(3).unwrap(),
            instant(18),
            TaskId::new("next-task").unwrap(),
        )
        .unwrap();
    assert_eq!(next.occurrence().unwrap().occurrence().offset(), 4);
    assert_eq!(next.next_offset(), Some(5));

    let future_task = TaskId::new("future-task").unwrap();
    let future = store
        .materialize_next_available_occurrence(
            &id,
            next.next_offset().unwrap(),
...
#[test]
fn materialize_next_available_advances_gaps_and_rejects_invalid_selection() {
    let directory = tempdir().unwrap();
    let path = directory.path().join("events.sqlite3");
    let id = RecurrenceId::new("materialize-next-gaps").unwrap();
    let missing = RecurrenceId::new("missing-materialize-next").unwrap();
    let mut store = RecurrenceStore::open(&path).unwrap();
    store
        .create(
            id.clone(),
            TaskGoal::new("Advance bounded materialization gaps").unwrap(),
            instant(1),
            ScheduleInterval::from_millis(1).unwrap(),
            OccurrenceCount::new(3).unwrap(),
        )
    ...
    let final_gap = store
        .materialize_next_available_occurrence(
            ...
            OccurrencePageSize::new(2).unwrap(),
            instant(u64::MAX),
            TaskId::new("final-gap-task").unwrap(),
        )
        .unwrap();
    assert_eq!(final_gap.occurrence(), None);
    assert_eq!(final_gap.next_offset(), None);
...
#[test]
fn materialize_next_available_preserves_occurrence_on_task_collision_and_read_only_failure() {
    let directory = tempdir().unwrap();
    let path = directory.path().join("events.sqlite3");
    let id = RecurrenceId::new("safe-materialize-next").unwrap();
    let task_id = TaskId::new("occupied-task").unwrap();
    let mut store = RecurrenceStore::open(&path).unwrap();
    store
        .create(
            id.clone(),
            TaskGoal::new("Materialize safely").unwrap(),
            instant(1),
            ScheduleInterval::from_millis(1).unwrap(),
            OccurrenceCount::new(1).unwrap(),
        )
    ...
    TaskStore::open(&path)
        .unwrap()
        .start(task_id.clone(), TaskGoal::new("Existing task").unwrap())
        .unwrap();
    assert!(matches!(
        store
            .materialize_next_available_occurrence(
            ...
            .unwrap_err(),
        RecurrenceStoreError::TaskAlreadyExists { task_id: existing } if existing == task_id
    ));
    assert_eq!(occurrence_event_count(&path, &id, 0), 1);
    drop(store);
    ...
    assert!(matches!(
        read_only
            .materialize_next_available_occurrence(
                ...
                OccurrencePageSize::new(1).unwrap(),
                instant(1),
                TaskId::new("read-only-task").unwrap(),
            )
            .unwrap_err(),
        RecurrenceStoreError::EventLog(_)
    ));
...
#[test]
fn concurrent_materialize_next_callers_bind_distinct_available_coordinates() {
    let directory = tempdir().unwrap();
    let path = directory.path().join("events.sqlite3");
    let id = RecurrenceId::new("concurrent-materialize-next").unwrap();
    let mut store = RecurrenceStore::open(&path).unwrap();
    store
        .create(
            id.clone(),
            TaskGoal::new("Materialize concurrently").unwrap(),
            instant(1),
            ScheduleInterval::from_millis(1).unwrap(),
            OccurrenceCount::new(2).unwrap(),
        )
...
#[test]
fn materialize_next_available_isolates_corruption_and_fails_closed_when_selected() {
    let directory = tempdir().unwrap();
    let path = directory.path().join("events.sqlite3");
    let id = RecurrenceId::new("materialize-next-corruption").unwrap();
    let unrelated_id = RecurrenceId::new("unrelated-materialize-next-corruption").unwrap();
    let mut store = RecurrenceStore::open(&path).unwrap();
    for recurrence_id in [&id, &unrelated_id] {
        store
            .create(
    ...
    let isolated = store
        .materialize_next_available_occurrence(
            ...
            OccurrencePageSize::new(3).unwrap(),
            instant(12),
            TaskId::new("isolated-task").unwrap(),
        )
        .unwrap();
    assert_eq!(isolated.occurrence().unwrap().occurrence().offset(), 0);
    assert!(matches!(
        store
            .materialize_next_available_occurrence(
                ...
                OccurrencePageSize::new(1).unwrap(),
                instant(13),
                TaskId::new("corrupt-task").unwrap(),
            )
            .unwrap_err(),
        RecurrenceStoreError::Replay(ReplayError::UnsupportedEvent { .. })
    ));
...
fn pages_sparse_claimed_occurrences_by_current_lifecycle_and_authored_window() {
    let directory = tempdir().unwrap();
    let path = directory.path().join("events.sqlite3");
    let id = RecurrenceId::new("sparse-claimed-page").unwrap();
    let mut store = RecurrenceStore::open(&path).unwrap();
    store
        .create(
            id.clone(),
            TaskGoal::new("Inspect sparse claimed provenance").unwrap(),
            instant(10),
            ScheduleInterval::from_millis(2).unwrap(),
            OccurrenceCount::new(7).unwrap(),
        )
    ...
    store
        .release_occurrence(
            ...
            3,
            2,
            RecurrenceOccurrenceRelease::new("return three").unwrap(),
        )
        .unwrap();
    store
        .materialize_claimed_occurrence(&id, 5, 2, TaskId::new("task-five").unwrap())
    ...
    store
        .release_occurrence(
            ...
            6,
            2,
            RecurrenceOccurrenceRelease::new("retry six").unwrap(),
        )
        .unwrap();
    store.claim_occurrence(&id, 6, 3, instant(22)).unwrap();
    drop(store);

    let store = RecurrenceStore::open_read_only(&path).unwrap();
    let first = store
        .claimed_occurrences_page(&id, 0, OccurrencePageSize::new(3).unwrap())
        .unwrap();
    assert_eq!(
        first
            .occurrences()
            .iter()
            .map(|claimed| (claimed.occurrence().offset(), claimed.revision()))
            .collect::<Vec<_>>(),
        vec![(0, 2), (2, 2)]
    );
    assert_eq!(first.next_offset(), Some(3));
    ...
    let gap = store
        .claimed_occurrences_page(&id, 3, OccurrencePageSize::new(2).unwrap())
        .unwrap();
    assert!(gap.occurrences().is_empty());
    assert_eq!(gap.next_offset(), Some(5));

    let final_page = store
        .claimed_occurrences_page(&id, 5, OccurrencePageSize::new(1024).unwrap())
        .unwrap();
    assert_eq!(
        final_page
            .occurrences()
...
#[test]
fn claimed_occurrence_pages_reject_missing_and_out_of_range_starts() {
    let directory = tempdir().unwrap();
    let mut store = RecurrenceStore::open(directory.path().join("events.sqlite3")).unwrap();
    let missing = RecurrenceId::new("missing-claimed-page").unwrap();
    assert!(matches!(
        store
            .claimed_occurrences_page(
                &missing,
                0,
                OccurrencePageSize::new(1).unwrap(),
            )
    ...
    let id = RecurrenceId::new("bounded-claimed-page").unwrap();
    store
        .create(
            id.clone(),
            TaskGoal::new("Reject invalid claimed starts").unwrap(),
            instant(1),
            ScheduleInterval::from_millis(1).unwrap(),
            OccurrenceCount::new(2).unwrap(),
        )
    ...
    assert!(matches!(
        store
            .claimed_occurrences_page(&id, 2, OccurrencePageSize::new(1).unwrap())
            .unwrap_err(),
        RecurrenceStoreError::OccurrenceOutOfRange {
            recurrence_id,
            requested_offset: 2,
...
#[test]
fn claimed_occurrence_pages_fail_closed_only_for_the_selected_window() {
    let directory = tempdir().unwrap();
    let path = directory.path().join("events.sqlite3");
    let id = RecurrenceId::new("claimed-page-corruption").unwrap();
    let unrelated_id = RecurrenceId::new("unrelated-claimed-page-corruption").unwrap();
    let mut store = RecurrenceStore::open(&path).unwrap();
    for recurrence_id in [&id, &unrelated_id] {
        store
            .create(
    ...
    for offset in 0..4 {
        store.persist_occurrence(&id, 1, offset).unwrap();
        store.claim_occurrence(&id, offset, 1, instant(13)).unwrap();
    }
    store.persist_occurrence(&unrelated_id, 1, 0).unwrap();
    store
        .claim_occurrence(&unrelated_id, 0, 1, instant(10))
        .unwrap();
    drop(store);
    ...
    for (recurrence_id, offset) in [(&id, 3_u64), (&unrelated_id, 0_u64)] {
        assert_eq!(
            connection
                .execute(
                    "UPDATE events SET payload_version = 2
                     WHERE stream_id = ?1 AND event_type = 'recurrence.occurrence_claimed'",
                    [format!(
                        "recurrence-occurrence:{}:{recurrence_id}:{offset}",
                        recurrence_id.as_str().len()
                    )],
    ...
    assert!(matches!(
        store
            .claimed_occurrences_page(&id, 3, OccurrencePageSize::new(1).unwrap())
            .unwrap_err(),
        RecurrenceStoreError::Replay(ReplayError::UnsupportedEvent { .. })
    ));
}
