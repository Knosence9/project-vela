...
pub fn infer_signals(input: &SuggestionInput) -> Result<SignalReport> {
    ...
    for event in &input.events {
            ...
            && !event.payload_json.trim().is_empty()
        {
            seen.insert(format!(
                "{}::{}",
                event.event_type,
                normalize_dedupe_text(&event.payload_json)
            ));
        }
    }
    ...
    let user_messages: Vec<&SuggestionMessage> =
    ...
    for message in &user_messages {
        if let Some(candidate_text) = infer_memory_candidate_from_message(&message.content) {
            ...
            let key = format!(
                "memory_signal::{}",
                normalize_dedupe_text(&payload.to_string())
            );
            if seen.insert(key) {
                signals.push(SignalSpec {
                    event_type: "memory_signal".to_string(),
    ...
    for message in &user_messages {
        if let Some((name, description, body)) = infer_skill_from_message(&message.content) {
            ...
            if seen.insert(key) {
                signals.push(SignalSpec {
                    event_type: "skill_signal".to_string(),
                    payload_json: payload.to_string(),
                });
            } else {
                skipped += 1;
        ...
        if let Some(candidate_text) = infer_memory_candidate_from_message(&message.content) {
            ...
            if memory_seen.contains(&key) {
                ...
                let candidate = stage_memory_candidate(
                    ...
                    Some(&candidate_text),
                    &format!(
                        "Transcript-derived preference from session {}.",
                        input.session_title
                    ),
    ...
    let report = match candidate.kind {
        ReviewKind::Memory => {
            let memory = candidate
                .memory
                .as_ref()
                .context("memory candidate payload missing")?;
        ...
        ReviewKind::Skill => {
            let skill = candidate
                .skill
                .as_ref()
                .context("skill candidate payload missing")?;
