...
fn default_backend_experiment_slots() -> Vec<BackendExperimentSlotRecord> {
    vec![
        ...
        BackendExperimentSlotRecord {
            id: "sparse-routing-preview".to_string(),
            status: "bounded-preview".to_string(),
            strategy: "shadow-routing".to_string(),
            summary: Some("Shadow a sparse-routing candidate through the durable eval harness so future routing experiments stay operator-visible without mutating the live route.".to_string()),
            hypothesis: Some("A sparse-routing preview lane can reveal whether a narrower backend handoff still preserves concise grounded behavior before any routing policy changes land.".to_string()),
            default_prompt: "Evaluate whether a sparse-routing candidate would keep this request concise, grounded, and reversible across the bounded backend set.".to_string(),
            allowed_backends: vec![
                "embedded".to_string(),
                "mock".to_string(),
                "llamacpp".to_string(),
        ...
        BackendExperimentSlotRecord {
            id: "adapter-intake-gate".to_string(),
            status: "bounded-preview".to_string(),
            strategy: "offline-replay".to_string(),
            summary: Some("Replay one bounded request across the published backends to judge whether a future adapter or fine-tune intake path would stay inside the current contract.".to_string()),
            hypothesis: Some("An adapter-intake lane can keep future backend intake criteria durable and reversible by comparing the current contract surfaces before any new route is promoted.".to_string()),
            default_prompt: "Replay this request across the bounded backends and note whether the current provider contract looks stable enough for a future adapter or fine-tune intake review.".to_string(),
            allowed_backends: vec![
                "embedded".to_string(),
                "llamacpp".to_string(),
                "mock".to_string(),
