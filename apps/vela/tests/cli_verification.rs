...
fn backend_eval_uses_configured_backend_when_no_backend_flag_is_supplied() {
    let vela_home = temp_vela_home("eval-default-configured-backend");
    std::fs::create_dir_all(&vela_home).unwrap();
    std::fs::write(
        vela_home.join("config.yaml"),
        "runtime:\n  provider: mock\n  model: mock-1\n",
    ...
    let ready = run_vela(&vela_home, &["eval"]);
    assert!(ready.status.success(), "{}", stderr_text(&ready));
    let ready_stdout = stdout_text(&ready);
    assert!(ready_stdout.contains("default_backend=mock"));

    let run = run_vela(&vela_home, &["eval", "--run", "compare configured backend"]);
    assert!(run.status.success(), "{}", stderr_text(&run));
    let run_stdout = stdout_text(&run);
    assert!(run_stdout.contains("backends=mock"));
    assert!(run_stdout.contains("backend=mock transport=in-process status=passed"));
...
fn backend_experiment_slot_uses_configured_backend_when_no_backend_flag_is_supplied() {
    let vela_home = temp_vela_home("eval-slot-default-configured-backend");
    std::fs::create_dir_all(&vela_home).unwrap();
    std::fs::write(
        vela_home.join("config.yaml"),
        "runtime:\n  provider: mock\n  model: mock-1\n",
    ...
    let run_slot = run_vela(
        &vela_home,
        &["eval", "--run-slot", "capability-parity-scan"],
    );
...
fn cron_report_summarizes_scheduler_state() {
    ...
    job["last_delivery_progression"] = serde_json::Value::from("delivery-failed");
    job["delivery_attempt_count"] = serde_json::Value::from(2);
    job["last_delivery_status_code"] = serde_json::Value::Null;
    ...
    assert!(report_stdout.contains("delivery_progression=Some(\"delivery-failed\")"));
    assert!(report_stdout.contains("delivery_attempt_count=2"));
    assert!(report_stdout.contains("delivery_status_code=None"));
    ...
    delivered_job["last_delivery_progression"] = serde_json::Value::from("delivery-delivered");
    delivered_job["last_delivery_status_code"] = serde_json::Value::from(200);
    ...
    assert!(delivered_stdout.contains("delivery_progression=Some(\"delivery-delivered\")"));
    assert!(delivered_stdout.contains("delivery_status_code=Some(200)"));
...
fn cron_start_executes_due_jobs() {
    ...
    assert!(show_stdout.contains("delivery_progression=Some(\"delivery-delivered\")"));
    assert!(show_stdout.contains("delivery_attempt_count=1"));
    assert!(show_stdout.contains("delivery_status_code=Some(200)"));
